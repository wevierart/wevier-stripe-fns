...
exports.handler = async (event) => {
  ...
  if (event.httpMethod !== 'POST') {
    return { statusCode: 405, headers: { ...CORS, Allow: 'OPTIONS, POST' }, body: 'Method Not Allowed' };
  }
  let total;
  try {
  ...
  const envHost = process.env.PAYPAL_ENV === 'production' ? 'api-m.paypal.com' : 'api-m.sandbox.paypal.com';
  ...
  try {
    ...
    return { statusCode: 502, headers: CORS, body: JSON.stringify({ error: 'Could not fetch PayPal token' }) };
  }
  ...
  try {
    ...
    return { statusCode: 502, headers: CORS, body: JSON.stringify({ error: 'Could not create PayPal order' }) };
  }
  return { statusCode: 200, headers: CORS, body: JSON.stringify({ id: orderJson.id }) };
};
